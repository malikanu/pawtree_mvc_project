var TestimonialPopup = {
	...
	var testimonialPopup = $('#write-testimonial');
        if (testimonialPopup.length == 0) {
	    
	    var popupHTML = '\
	    <div id="write-testimonial" class="modal fade" role="dialog">\
		<div class="modal-dialog" role="document"><div class="modal-content">\
			...
			<div class="step1">\
			    ...
			    <form method="post" action="/testimonial/submit" target="testimonial_target" enctype="multipart/form-data">\
				<div class="row">\
				    <div class="col-sm-6">\
				...
				<div class="row">\
				    ...
				    <div class="col-sm-6">\
					<div class="form-group">\
					    <label for="testimonial_petname">Pet\'s Name(s)</label>\
					    <input type="text" class="form-control" name="petname" id="testimonial_petname" placeholder="" required="required">\
					</div>\
				    </div>\
				</div>\
				<div class="row">\
	...
	// show popup
	$('.add-testimonial').on('click',TestimonialPopup.showPopup);
	
	// make form submit
	$("#write-testimonial .step1 .save").on("click",function() {
	    $('#write-testimonial .step1 form').submit();
	    $('#write-testimonial .step1').hide();
	    $('#write-testimonial .progress').show();
	    $('#write-testimonial .response').hide(); // previous response
	});
	
	// clear validation
	$('#write-testimonial #testimonial_name').on('change keydown',function() {
	    $(this).removeClass('error');
	});
	$('#write-testimonial #testimonial_testimonial').on('change keydown',function() {
	    $(this).removeClass('error');
	});
	...
	$('#write-testimonial .progress').hide();
	$('#write-testimonial .response').hide();
	$('#write-testimonial .step1 form').trigger('reset');
	$('#write-testimonial .response').html('');
	$('#write-testimonial .step1').show();
	
	$('#write-testimonial #testimonial_testimonial').removeClass('error');
	$('#write-testimonial #testimonial_name').removeClass('error');
	
    },
    
    showPopup: function() {
	
	TestimonialPopup.reset();
	...
	setTimeout(function() {
		$('#write-testimonial .select2:not(.init)').select2({
	    	placeholder: "SELECT"
	    });
	    $('#write-testimonial .select2-multi:not(.init)').select2({
	    	placeholder: "SELECT ALL THAT APPLY"
	    });
	    $('#write-testimonial .select2').addClass('init');
	    $('#write-testimonial .select2-multi').addClass('init');
	},1000)
	...
	$('#write-testimonial .response').html('Thank you! Your testimonial will appear on the website soon.');
	$('#write-testimonial .progress').hide();
	$('#write-testimonial .response').show();
	...
	$('#write-testimonial .response').html('Thank you! Your testimonial has been saved, please wait while we process the video...<hr/><div id="#videoUploaded">...</div>');
	$('#write-testimonial .progress').hide();
	$('#write-testimonial .response').show();
	...
	$('#write-testimonial .step1').show();
	$('#write-testimonial .response').html('<ul class="errorMsgs"><li>Please complete the required fields.</li></ul>');
	$('#write-testimonial .progress').hide();
	$('#write-testimonial .response').show();
	
	if ($('#write-testimonial #testimonial_name').val() == '') {
	    $('#write-testimonial #testimonial_name').addClass('error');
	}
	
	if ($('#write-testimonial #testimonial_title').val() == '') {
	    $('#write-testimonial #testimonial_title').addClass('error');
	}
	
	if ($('#write-testimonial #testimonial_testimonial').val() == '') {
	    $('#write-testimonial #testimonial_testimonial').addClass('error');
	}
    
    if ($('#write-testimonial #testimonial_photo').val() == '') {
	    $('#write-testimonial #testimonial_photo').addClass('error');
	}
	...
	$('#write-testimonial .step1').show();
	$('#write-testimonial .response').html('<ul class="errorMsgs"><li>Internal server error, unable to save data. Please try again.</li></ul>');
	$('#write-testimonial .progress').hide();
	$('#write-testimonial .response').show();
	...
	$('#write-testimonial .step1').show();
	$('#write-testimonial .response').html('<ul class="errorMsgs"><li>Internal server error, unable to save photo. Please try again.</li></ul>');
	$('#write-testimonial .progress').hide();
	$('#write-testimonial .response').show();
...
$(document).ready(TestimonialPopup.init);